...
class RadioPoller(threading.Thread):
    ...
    def _digest(self, s, a):
        ...
        # band noise floor: [freq_MHz, worst-antenna avg dBm, burst spread dB]
        # spread = RssiMax - RssiAvg: high spread + low avg = pulsed
        # interferer (FEC absorbs it); high avg = continuous (hop away).
        # Note: enabling Noise scan on the radio's Spectral page doubles
        # resolution to 5 MHz steps in this same field.
        noise = []
        for n in s.get("noiseRssi", []):
            try:
                a1 = n.get("ant1Rssi", -120.0)
                a2 = n.get("ant2Rssi", -120.0)
                avg = max(a1, a2)
                spread = max((n.get("ant1RssiMax") or a1) - a1,
                             (n.get("ant2RssiMax") or a2) - a2, 0)
                noise.append([round(n["freq"] / 1e6), round(avg, 1),
                              round(spread, 1)])
...
class Store:
    def __init__(self, path):
        ...
        for candidate in (path, fallback, ":memory:"):
            try:
                ...
                try:
                    db.execute("ALTER TABLE runs ADD COLUMN summary TEXT")
    ...
    def end_run(self, run_id, summary=None):
        with self.lock:
    ...
    def samples_for(self, run_id):
        with self.lock:
            rows = self.db.execute("SELECT data FROM samples WHERE run_id = ?"
    ...
    def runs(self):
        with self.lock:
            rows = self.db.execute(
                ...
                "summary FROM runs ORDER BY id DESC LIMIT 100").fetchall()
        return [{"id": r[0], "started": r[1], "ended": r[2],
                 "direction": r[3], "transport": r[4],
                 "params": json.loads(r[5] or "{}"),
                 "summary": json.loads(r[6] or "{}")} for r in rows]
...
def _summarize(samples, target_fps):
    """Per-run scoreboard: how watchable was the feed, and why."""
    delivery, snr = [], []
    arq = lost = 0.0
    ovf = 0
    freeze = n_fps = 0
    prev_rendered = None
    for s in samples:
        fps = (s.get("pipeline") or {}).get("fps") or {}
        if fps.get("current") is None:
            fps = ((s.get("peer") or {}).get("pipeline") or {}).get("fps") or {}
        cur = fps.get("current")
        if cur is not None and target_fps:
            delivery.append(min(cur / target_fps * 100, 110))
        rend = fps.get("rendered")
        if rend is not None:
            if prev_rendered is not None and rend <= prev_rendered:
                freeze += 1
            prev_rendered = rend
            n_fps += 1
        r = s.get("radio") or {}
        so, si = r.get("snr_out"), r.get("snr_in")
        if isinstance(so, (int, float)) and isinstance(si, (int, float)):
            snr.append(min(so, si))
        arq += r.get("arq_d") or 0
        lost += r.get("rx_lost_d") or 0
        ovf = max(ovf, r.get("tx_overflow") or 0)
    avg = lambda lst: round(sum(lst) / len(lst), 1) if lst else None
    return {"seconds": len(samples),
            "delivery_avg": avg(delivery),
            "delivery_min": round(min(delivery), 1) if delivery else None,
            "freeze_pct": round(freeze / n_fps * 100, 1) if n_fps else None,
            "snr_avg": avg(snr),
            "snr_min": round(min(snr), 1) if snr else None,
            "arq_total": round(arq), "rx_lost_total": round(lost),
            "tx_overflow": ovf}
...
class Agent:
    ...
    def run_start(self, body):
        ...
        video = pipelines.merge_params(pipelines.VIDEO_PARAMS,
                                       body.get("video") or {})
    ...
    def run_stop(self):
        if self.run:
            run = self.run
            self.run = None
            summary = _summarize(self.store.samples_for(run["id"]),
                                 run.get("target_fps") or 30)
            self.store.end_run(run["id"], summary)
        self.pipe.stop()
        try:
